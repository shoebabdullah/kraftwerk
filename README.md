**Professional Certificate in Machine Learning and Artificial Intelligence**
<br>**Practical Application 3**<br /> 
<br>**Required Assignment 17.1: Comparing Classifiers**<br /> 
<br># Bank Marketing Campaign: Comparing Classifiers<br /> 

<br>## Project Overview<br /> 
<br>The objective of this project is to develop a predictive model that accurately identifies the likelihood of a client subscribing to a bank term deposit. Using a <br>dataset from a Portuguese banking institution (UCI Machine Learning Repository), we compare the performance of four primary classifiers:<br /> 
<br>* **K-Nearest Neighbors (KNN)**<br /> 
<br>* **Logistic Regression**<br /> 
<br>* **Decision Trees**<br /> 
<br>* **Support Vector Machines (SVM)**<br /> 
